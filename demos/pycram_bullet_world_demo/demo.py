...
if robot.name == "iCub":
    ...
    bowl = Object("bowl", ObjectType.BOWL, "bowl.stl", pose=Pose([4.7, 4.5, 0.55], [0, 0, -1, 1]),
                  color=Color(1, 1, 0, 1))
    ...
    cereal = Object("cereal", ObjectType.BREAKFAST_CEREAL, "breakfast_cereal.stl",
                    pose=Pose([2.5, 2.5, 1.05], [0, 0, 1, 1]), color=Color(0, 1, 0, 1))
    cereal_target_pose = Pose([5.2, 3.3, 0.81], [0, 0, 1, 1])

    bowl = Object("bowl", ObjectType.BOWL, "bowl.stl", pose=Pose([2.4, 2.2, 0.98], [0, 0, 1, 1]),
                  color=Color(1, 1, 0, 1))
...
with simulated_robot:
    ...
    #
    # TransportAction(milk_desig, [Arms.RIGHT], [milk_target_pose]).resolve().perform()

    cereal_desig = move_and_detect(ObjectType.BREAKFAST_CEREAL)

    TransportAction(cereal_desig, [Arms.RIGHT], [cereal_target_pose]).resolve().perform()
